                    ...
                    <div class="mt-4 overflow-x-auto">
                        <div id="accordion-open" data-accordion="open">
                            @foreach ($perusahaan as $group)
                                <h2 id="accordion-open-heading-1{{ $loop->iteration }}">
                                    <button type="button" class="{{ $loop->first ? 'rounded-t-xl' : '' }} {{ $loop->last ? 'rounded-b-xl' : '' }} bg-white-custom flex items-center justify-between w-full p-5 font-medium rtl:text-right text-gray-500 border border-b-0 border-gray-300  focus:ring-4 focus:ring-gray-300 gap-3" 
                                        data-accordion-target="#accordion-open-body-1{{ $loop->iteration }}" 
                                        aria-expanded="{{ $loop->first ? 'true' : 'false' }}" 
                                        aria-controls="accordion-open-body-1{{ $loop->iteration }}">
                                            <span class="flex items-center"> {{ $group['nama_perusahaan'] }}</span>
                                            <svg data-accordion-icon class="w-3 h-3 rotate-180 shrink-0" aria-hidden="true" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 10 6">
                                                <path stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 5 5 1 1 5"/>
                                            </svg>
                                    </button>
                                </h2>
                                <div id="accordion-open-body-1{{ $loop->iteration }}" class="hidden" aria-labelledby="accordion-open-heading-1{{ $loop->iteration }}">
                                    <div class="p-5 border border-b-0 border-gray-200 dark:border-gray-700 dark:bg-gray-900 overflow-auto">
                                        <table>
                                            <thead class=" ">
                                                <tr>
                                                    ...
                                                    @endif
                                                    <th class="px-6 py-3 border-b border-gray-200 text-gray-800 text-left text-xs font-semibold uppercase tracking-wider">No</th>
                                                    <th class="px-6 py-3 border-b border-gray-200 text-gray-800 text-left text-xs font-semibold uppercase tracking-wider">Nama</th>
                                                    <th class="px-6 py-3 border-b border-gray-200 text-gray-800 text-left text-xs font-semibold uppercase tracking-wider">Unit Kerja</th>
                                                    <th class="px-6 py-3 border-b border-gray-200 text-gray-800 text-left text-xs font-semibold uppercase tracking-wider">Area</th>
                                                    <th class="px-6 py-3 border-b border-gray-200 text-gray-800 text-left text-xs font-semibold uppercase tracking-wider">No KTP</th>
                                                    <th class="px-6 py-3 border-b border-gray-200 text-gray-800 text-left text-xs font-semibold uppercase tracking-wider">Berapa Lama Bekerja</th>
                                                    <th class="px-6 py-3 border-b border-gray-200 text-gray-800 text-left text-xs font-semibold uppercase tracking-wider">Tanggal Mulai Bekerja</th>
                                                    <th class="px-6 py-3 border-b border-gray-200 text-gray-800 text-left text-xs font-semibold uppercase tracking-wider">Status</th>
                                                    <th class="px-6 py-3 border-b border-gray-200 text-gray-800 text-left text-xs font-semibold uppercase tracking-wider">Actions</th>
                                                </tr>
                                            </thead>
                                            <!-- TABLE BODY -->
                                            <tbody class="bg-white font-semibold tracking-wider ">
                                                ...
                                                @if (Auth::user()->role != 'admin' && Auth::user()->role != 'user')
                                                    <form action="{{ route('pengajuan.manyApprove') }}" method="POST">
                                                        @csrf
                                                @endif  
                                                ...
                                                @foreach ($group['pengajuans'] as $pengajuan)
                                                    <tr>
                                                        ...
                                                        <!-- dtail anchor button and delete modal -->
                                                ...
                                                @if (Auth::user()->role != 'admin' && Auth::user()->role != 'user')
                                                        <tr>
                                                            <td colspan="12">
                                                                <button data-modal-target="approve_modal{{ $loop->iteration }}" data-modal-toggle="approve_modal{{ $loop->iteration }}" class="text-sm bg-orange text-white px-4 py-2 rounded mt-5" type="button">
                                                                    Approve
                                                                </button>
                                                            </td>
                                                        </tr>
                                                        <div id="approve_modal{{ $loop->iteration }}" tabindex="-1" class="hidden overflow-y-auto overflow-x-hidden fixed top-0 right-0 left-0 z-50 justify-center items-center w-full md:inset-0 h-[calc(100%-1rem)] max-h-full">
                                                            <div class="relative p-4 w-full max-w-md max-h-full">
                                                                <div class="relative bg-white rounded-lg shadow dark:bg-gray-700">
                                                                    <button type="button" class="absolute top-3 end-2.5 text-gray-400 bg-transparent hover:bg-gray-200 hover:text-gray-900 rounded-lg text-sm w-8 h-8 ms-auto inline-flex justify-center items-center dark:hover:bg-gray-600 dark:hover:text-white" data-modal-hide="approve_modal{{ $loop->iteration }}">
                                                                        <svg class="w-3 h-3" aria-hidden="true" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 14 14">
                                                                            <path stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="m1 1 6 6m0 0 6 6M7 7l6-6M7 7l-6 6"/>
                                                                        </svg>
                                                                        <span class="sr-only">Close modal</span>
                                                                    ...
                                                                    <div class="p-4 md:p-5 text-center">
                                                                        ...
                                                                        <button data-modal-hide="approve_modal{{ $loop->iteration }}" type="submit" class="text-white bg-red-600 hover:bg-red-800 focus:ring-4 focus:outline-none focus:ring-red-300 dark:focus:ring-red-800 font-medium rounded-lg text-sm inline-flex items-center px-5 py-2.5 text-center">
                                                                            Yes, I'm sure
                                                                        </button>
                                                                        <button data-modal-hide="approve_modal{{ $loop->iteration }}" type="button" class="py-2.5 px-5 ms-3 text-sm font-medium text-gray-900 focus:outline-none bg-white rounded-lg border border-gray-200 hover:bg-gray-100 hover:text-blue-700 focus:z-10 focus:ring-4 focus:ring-gray-100 dark:focus:ring-gray-700 dark:bg-gray-800 dark:text-gray-400 dark:border-gray-600 dark:hover:text-white dark:hover:bg-gray-700">No, cancel</button>
                                                                    </div>
                                                                </div>
                                                            </div>
                                                        </div>
